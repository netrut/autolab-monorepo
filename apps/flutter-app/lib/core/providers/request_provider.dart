...
class RequestProvider extends ChangeNotifier {
  ...
  /// Send a service centre join request.
  Future<bool> sendServiceCenterJoinRequest({
    required String serviceCenterId,
    String? message,
  }) async {
    return _send({
      'type': RequestType.serviceCenterJoin.value,
  ...
  Future<bool> _send(Map<String, dynamic> data) async {
    try {
      final res = await _api.post('/api/requests', data: data);
      final req = RequestModel.fromJson(res.data as Map<String, dynamic>);
      // Add to sent list if not already there
      if (!_sent.any((r) => r.id == req.id)) {
        _sent.insert(0, req);
        notifyListeners();
      }
      return true;
  ...
  Future<bool> _updateStatus(
      String id, String action, List<RequestModel> list) async {
    try {
      final res = await _api.put('/api/requests/$id/$action');
      final updated =
          RequestModel.fromJson(res.data as Map<String, dynamic>);
