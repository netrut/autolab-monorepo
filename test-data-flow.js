...
async function runTests() {
  ...
  try {
    ...
    if (backendRes.status === 200) {
      ...
      console.log(`✅ Success! Found ${Array.isArray(vehicles) ? vehicles.length : 'N/A'} vehicle(s)\n`);
      
      if (Array.isArray(vehicles) && vehicles.length > 0) {
        console.log('📋 Backend Vehicle Data:');
        vehicles.forEach((v, idx) => {
          console.log(`   [${idx + 1}] ${v.brand} ${v.model} (${v.year}) - ${v.registration_number}`);
          console.log(`       Color: ${v.vehicle_color}, Type: ${v.vehicle_type}`);
        });
      }
    } else {
  ...
  console.log('-'.repeat(70));
  try {
    const dashboardRes = await makeRequest('http://localhost:3000/api/vehicles');
    console.log(`Status: ${dashboardRes.status}`);
    
    if (dashboardRes.status === 200) {
      const vehicles = JSON.parse(dashboardRes.body);
      console.log(`✅ Success! Proxy returned ${Array.isArray(vehicles) ? vehicles.length : 'N/A'} vehicle(s)\n`);
  ...
  try {
    ...
    if (codespaceRes.status === 200) {
      ...
      console.log(`✅ Success! Codespace URL returned ${Array.isArray(vehicles) ? vehicles.length : 'N/A'} vehicle(s)\n`);
